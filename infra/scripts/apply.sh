...
case "$ACTION" in
  ...
  apply)
    ...
    echo ""
    echo "Next steps:"
    echo "  1. wrangler d1 migrations apply fork-${ENV} --remote --env ${ENV} -c wrangler.toml -c infra/generated/wrangler.${ENV}.toml"
    echo "  2. wrangler secret bulk infra/secrets.${ENV}.env --env ${ENV} -c wrangler.toml -c infra/generated/wrangler.${ENV}.toml"
